...
- **416 tests passing** across 19 test files with **877 expect() calls**
...
- **subscription-flow.md**: All flows correct. Email validation, subscriber cap, rate limiting, existing subscriber lookup all working.
- **fact-cycling.md**: Algorithm correct. All edge cases tested.
- **seed-data.md**: `sync-facts.ts` correctly syncs, upserts by text match, generates images via DALL-E.
- **fact-images.md**: AI generation at sync time, graceful degradation when no image.
...
- **email-mascot.md**: Mascot PNG in email headers, emoji removed from HTML. `emailWrapper()` accepts `baseUrl`.
