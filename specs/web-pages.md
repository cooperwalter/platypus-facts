...
- **Repeating platypus background pattern** — all pages use a subtle, tiled background of a small platypus line icon (inline SVG or a small static asset). The icons should be low-opacity so they don't compete with page content, creating a soft watermark-style texture across the entire page.
- An **animated swimming platypus** cartoon that roams the entire page. The platypus should swim freely across the full page area — up, down, left, and right — in a looping path that covers the whole viewport, not confined to a small section or strip. The animation should be implemented with CSS (keyframe animations on a fixed/absolute-positioned SVG or image element) — no JavaScript animation libraries, no GIFs, no external dependencies. Movement must be **smooth and continuous** — use `linear` timing so the platypus glides at a constant speed without jerks or pauses at keyframe boundaries. The platypus must **face the direction it is moving** — flip horizontally (e.g., `scaleX(-1)`) when switching between leftward and rightward travel, applied at the same keyframe as the position change so it never appears to swim backwards. It should float behind the page content (lower z-index) so it doesn't interfere with form inputs or links.
